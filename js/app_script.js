...
document.addEventListener('DOMContentLoaded', async function() {
        ...
        if (!currentRequest && canSubmit) {
            ...
            container.innerHTML = `
                <div class="request-info">
                    ...
                    ${statusCode === 'rejected' && canSubmit ? `<button class="application-button mt-10" id="new-request-btn">Подать новую заявку</button>` : ''}
                </div>
            `;
            if (canCancel) {
                document.getElementById('cancel-request').addEventListener('click', handleCancelRequest);
            }
            if (statusCode === 'rejected' && canSubmit) {
        ...
        container.innerHTML = `
            <div class="practice-info">
                ...
                <div class="practice-details">
                    ...
                    <p><strong>Место обучения:</strong></p>
    ...
    async function handleCancelRequest() {
        ...
        const result = await updateRequestStatus(currentRequest.id, 'rejected');
        if (result.success) {
            await loadRequests();
